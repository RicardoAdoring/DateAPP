...
- 🧩 **Provider 隔离**：LLM、Embedding、Rerank、高德、SearchApi 都由后端环境变量管理，前端不暴露服务端密钥。
...
| LLM | DeepSeek OpenAI-compatible API，当前模型 `deepseek-v4-flash` |
...
数据流路径：前端完成地点选择和约会偏好输入 → 后端标准化中心点 → 高德 MCP 收集周边 POI → SearchApi.io 补充可信度信息 → RAG 提供本地攻略上下文 → DeepSeek 生成结构化 itinerary → 高德 MCP 估算相邻路线 → 前端流式展示并进入结果页。
